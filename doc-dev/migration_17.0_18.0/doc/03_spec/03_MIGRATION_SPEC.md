...
| `tests/test_product_history_report.py:27` — `cls.env['product.template'].create({..., 'type': 'product'})` | DIFF-12 (ditemukan lewat eksekusi G1, bukan review statis) | **Wajib update** `'type': 'product'` → `'is_storable': True` (biarkan `type` default `'consu'`) — `type='product'` sudah dihapus dari selection 18.0, `stock` sekarang pakai field terpisah `is_storable` untuk produk yang di-track stoknya | Rendah — mekanis, tapi WAJIB (blocker `setUpClass`, semua 8 test gagal kalau tidak difix) | Semua AC (setup fixture dipakai semua test) |
