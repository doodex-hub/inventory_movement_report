...
|---|---|---|---|---|
| `security/ir.model.access.csv` → **hapus**; **baru** `security/ir.access.csv` | DIFF-01 | Isi persis output konversi resmi (`odoo/upgrade_code/19.4-00-ir-access.py`): header `id,name,model_id,group_id/id,operation,domain`, baris `access_stock_history_view,stock_history_view,stock.history.view,base.group_everyone,crud,`. XML-ID `access_stock_history_view` DIPERTAHANKAN (tidak rename). Verifikasi silang: jalankan skrip resmi `odoo-bin upgrade_code --script 19.4-00-ir-access` terhadap salinan modul 19.0 di container dan bandingkan output (Fase A3). | Tinggi → rendah setelah verifikasi skrip resmi | BSL-007, BSL-008 |
...
| `models/product_template.py`, `models/stock_history_view.py`, `models/__init__.py`, `controllers/__init__.py`, `controllers/controllers.py`, `__init__.py`, `tests/__init__.py` | DIFF-05, 07, 08, 09 | **Tidak diubah** | Rendah | BSL-001..006, 009, 011, 014 |
| `LICENSE`, `README.md`, `LISEZMOI.md`, `googleaeed8a7b9ec156e7.html` (file verifikasi Google Search Console) di dalam `product_history_report/` | — | **Tidak diubah** (non-kode) — KECUALI baris "Odoo version" README/LISEZMOI "17.0" → "20.0" di Fase A6 (housekeeping wajib `06a`). *(Baris ditambahkan saat Step 4 — gap cakupan; dikoreksi Step 6.)* | Rendah | — |
